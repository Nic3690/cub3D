...
/*   Updated: 2024/07/21 13:48:39 by nfurlani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    double sprite_x = door->pos_x - game->pg->pos_x;
    double sprite_y = door->pos_y - game->pg->pos_y;
    ...
    for (int stripe = draw_start_x; stripe < draw_end_x; stripe++)
    {
        int tex_x = (int)(256 * (stripe - (-sprite_width / 2 + sprite_screen_x)) * door->curr_tex->w / sprite_width) / 256;
        if (transform_y > 0 && stripe > 0 && stripe < WIDTH && transform_y < game->z_buffer[stripe])
        {
            for (int y = draw_start_y; y < draw_end_y; y++)
            {
                int d = (y) * 256 - HEIGHT * 128 + sprite_height * 128;
                int tex_y = ((d * door->curr_tex->h) / sprite_height) / 256;

                if (tex_x >= 0 && tex_x < door->curr_tex->w && tex_y >= 0 && tex_y < door->curr_tex->h)
                {
                    int color = get_tex_color(door->curr_tex, tex_x, tex_y);
                    if (color != (0xFF << 24))
                        pixel_put(game, stripe, y, color);
                }
            }
