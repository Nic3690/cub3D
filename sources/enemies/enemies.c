...
/*   Updated: 2024/07/28 12:57:44 by nfurlani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    distance_between_enemies(t_game *game, int i, double min_dist_btw)
{
    ...
    while (j < game->num_enemies)
    {
        ...
        {
            double dist_x = game->enemies[i].pos_x - game->enemies[j].pos_x;
            double dist_y = game->enemies[i].pos_y - game->enemies[j].pos_y;
            double dist_btw = sqrt(dist_x * dist_x + dist_y * dist_y);
            if (dist_btw < min_dist_btw)
            {
                double overlap = min_dist_btw - dist_btw;
...
void    init_enemy_rays(t_game *game, t_enemy *enemy)
{
    ...
    enemy->dist = sqrt(enemy->ray_dir_x * enemy->ray_dir_x + enemy->ray_dir_y * enemy->ray_dir_y);
...
void    perform_enemy_dda(t_enemy *enemy)
{
    ...
    {
        enemy->step_x = 1;
        enemy->side_dist_x = (enemy->map_x + 1.0 - enemy->pos_x) * enemy->delta_x;
    }
    ...
    {
        enemy->step_y = 1;
        enemy->side_dist_y = (enemy->map_y + 1.0 - enemy->pos_y) * enemy->delta_y;
    }
}
