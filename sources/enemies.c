...
/*   Updated: 2024/07/13 23:58:59 by nfurlani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void render_sprite(t_game *game, t_enemy *enemy)
{
    ...
    {
        int tex_x = (int)(256 * (stripe - (-sprite_width / 2 + sprite_screen_x)) * enemy->texture->w / sprite_width) / 256;
        if (transform_y > 0 && stripe > 0 && stripe < WIDTH && transform_y < game->z_buffer[stripe])
        {
            for (int y = draw_start_y; y < draw_end_y; y++)
            {
                int d = (y) * 256 - HEIGHT * 128 + sprite_height * 128;
                int tex_y = ((d * enemy->texture->h) / sprite_height) / 256;
                int color = get_tex_color(enemy->texture, tex_x, tex_y);
                if (color != (0xFF << 24))
